...
let count = 0 //# of toal words

//callback function for iterating over storyWords array
storyWords.forEach((word) => {
    count++;
})
console.log(count)

//removing the unnecessaryWord via the filter method
storyWords = storyWords.filter(word => {
    return word != unnecessaryWord
})
//finding the mispelling 'beautifull' and replacing with 'beautiful'
storyWords = storyWords.map((word) => {
    return word === misspelledWord ? 'beautiful' : word;
})
//finding index of the badWord in order to replace it
...
//boolean checking if there are any words over 10 characters
let lengthCheck = storyWords.every((word) => {
    return word.length < 10;
})
console.log(lengthCheck)
//if there is a word over 10 characters, log that word . Manually changed the word
storyWords.forEach((word) => {
    word.length > 10 && console.log(word)
})
//combining the storyWords for readability in console
